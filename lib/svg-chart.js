...
export function generateSVGChart({ months, data, title }) {
  ...
  const legendItems = [
    ...
    { label: "Phone", color: COLORS.phone },
    { label: "CW", color: COLORS.cw },
    { label: "Digital", color: COLORS.digital },
  ];
  for (let i = 0; i < legendItems.length; i++) {
    const { label, color } = legendItems[i];
    const lx = legendX + i * 100;
    svg += `  <rect x="${lx}" y="${legendY}" width="20" height="3" fill="${color}"/>\n`;
    svg += `  <text x="${lx + 24}" y="${legendY + 4}" font-size="11" fill="#333">${label}</text>\n`;
  }
