...
function generateGrowthChart(buckets, callsign) {
  ...
  // 按月聚合：收集所有桶中每个实体的 QSL 日期，按月统计新增数
  ...
  // 每月一个数据点（精确曲线），但标签只在 6 月和 12 月显示文字
  const labels = continuousMonths.map((month) => {
    const mo = parseInt(month.split("-")[1]);
    return (mo === 6 || mo === 12) ? month : "";
  });
  ...
  for (const bid of MODE_BUCKETS) {
    let cumulative = 0;
    timelineData[bid] = [];
    for (const m of continuousMonths) {
      cumulative += bucketTimelines[bid][m] || 0;
      timelineData[bid].push(cumulative);
    }
  }
