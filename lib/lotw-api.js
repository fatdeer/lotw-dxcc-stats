...
async function requestWithRetry(
  ...
  // 默认重试间隔：5分钟 → 10分钟 → 20分钟
  const delays = retryDelays || [5 * 60 * 1000, 10 * 60 * 1000, 20 * 60 * 1000];
  let lastError;
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (isRetryable && attempt < maxRetries) {
        const delayTime = delays[attempt - 1] || delays[delays.length - 1];

        console.log(
          `[LoTW-DXCC] LoTW request failed (attempt ${attempt}/${maxRetries}): ${error.message} [${error.code || status || 'unknown'}]`,
        );
        console.log(
          `[LoTW-DXCC] Waiting ${Math.round(delayTime / 60000)} minutes before retry...`,
        );

        if (progress) {
          progress.updateMessage(
            `Request failed, waiting ${Math.round(delayTime / 60000)} min before retry (${attempt}/${maxRetries})`,
          );
        }
...
export async function fetchADIFData({
  ...
  // 优先使用传入的呼号，其次从 configContext 读取（多呼号模式），最后回退到 username
  const ownCall = (
    paramOwnCall ||
    (configContext.getCallsign && configContext.getCallsign()) ||
    username
  )
    .toString()
    .toUpperCase();
  ...
  console.log(
    `[LoTW-DXCC] Lotw-update: Starting to fetch Full ADIF data from LoTW for callsign: ${ownCall}...`,
  );
  console.log(
  ...
  try {
    return await _fetchDataFromLoTW({
      ...
      configContext,
      progressMessage: `Fetching full ADIF data (${ownCall})`,
      onDownloadProgress: (progressEvent, progress) => {
...
export async function fetchIncrementalQSL({
  ...
  console.log(
    `[LoTW-DXCC] Lotw-update: Fetching incremental QSL data for ${ownCall} since: ${qslSince}`,
  );
  ...
  try {
    return await _fetchDataFromLoTW({
      ...
      configContext,
      progressMessage: `Fetching incremental QSL data (${ownCall})`,
      onDownloadProgress: (progressEvent, progress) => {
...
export async function fetchIncrementalQSO({
  ...
  console.log(
    `[LoTW-DXCC] Lotw-update: Fetching incremental QSO data for ${ownCall} since: ${qsoSince}`,
  );
  ...
  try {
    return await _fetchDataFromLoTW({
      ...
      configContext,
      progressMessage: `Fetching incremental QSO data (${ownCall})`,
      onDownloadProgress: (progressEvent, progress) => {
        const loadedKB = Math.round(progressEvent.loaded / 1024);
