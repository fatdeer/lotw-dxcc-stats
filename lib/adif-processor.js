...
/**
 * Extract an ADIF field value using its declared length.
 */
function extractADIFFieldValue(adifText, fieldName) {
  const escapedFieldName = fieldName.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  const fieldMatch = adifText.match(
    new RegExp(`<${escapedFieldName}:(\\d+)(?::[^>]*)?>`, "i"),
  );
  ...
  const valueLength = parseInt(fieldMatch[1], 10);
  return adifText.substring(valueStart, valueStart + valueLength).trim();
}

/**
 * Replace an ADIF header field while preserving text after its declared value.
 */
function setADIFHeaderField(header, fieldName, value) {
  const escapedFieldName = fieldName.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  const fieldMatch = header.match(
    new RegExp(`<${escapedFieldName}:(\\d+)(?::[^>]*)?>`, "i"),
  );
  const replacement = `<${fieldName}:${value.length}>${value}`;

  if (!fieldMatch) {
    return header.replace(/<eoh>/i, `${replacement}\n<eoh>`);
  }

  const valueStart = fieldMatch.index + fieldMatch[0].length;
  const valueLength = parseInt(fieldMatch[1], 10);
  return (
    header.substring(0, fieldMatch.index) +
    replacement +
    header.substring(valueStart + valueLength)
  );
}
...
export async function mergeIncrementalADIFData(
  ...
  // Advance the QSO cursor to the newest value returned by LoTW. Without this,
  // every scheduled run downloads and appends the same incremental records.
  const latestQsoRx = extractADIFFieldValue(
    incrementalHeader,
    "APP_LoTW_LASTQSORX",
  );
  if (latestQsoRx) {
    updatedHeader = setADIFHeaderField(
      updatedHeader,
      "APP_LoTW_LASTQSORX",
      latestQsoRx,
    );
  ...
  // Parse both sides and skip records already present locally. The cursor update
  // is the primary protection; identity-based filtering also makes retries and
  // overlapping LoTW responses idempotent.
  ...
  const existingKeys = new Set(
    existingRecordsList.map((record) => buildQSOIdentityKey(record)),
  );
  const newRecords = [];

  for (const record of incrementalRecordsList) {
    const identityKey = buildQSOIdentityKey(record);
  ...
  const mergedRecords = [...newRecords, ...existingRecordsList];
  const recordCount = mergedRecords.length.toString();
  updatedHeader = setADIFHeaderField(
    updatedHeader,
    "APP_LoTW_NUMREC",
    recordCount,
  ...
  console.log(
    `[LoTW-DXCC] ADIF-processor: Added ${newRecords.length} new QSO records, skipped ${incrementalRecordsList.length - newRecords.length} duplicates`,
  );

  const mergedData = updatedHeader + rebuildRecordsSection(mergedRecords);
...
/**
 * Build a stable identity for a LoTW QSO. APP_LoTW_QSO_TIMESTAMP is the QSO
 * date/time and can collide, so include the worked callsign and core QSO fields.
 */
function buildQSOIdentityKey(recordText) {
  ...
  const values = identityFields.map(
    (fieldName) => extractADIFFieldValue(recordText, fieldName) || "",
  );
  ...
  return `raw:${recordText.replace(/\s+/g, " ").trim()}`;
}
...
function extractTimestampFromRecord(recordText) {
  return extractADIFFieldValue(recordText, "APP_LoTW_QSO_TIMESTAMP");
}
