...
function generateGrowthChart(buckets, callsign) {
  ...
  // 使用半年刻度点（每年 6 月和 12 月），每个点的值是截至该月的精确累计数
  // Mermaid xychart 的 x-axis 不支持空字符串标签，所以只放有意义的刻度
  ...
  // 只保留 <= lastMonth 的点
  const labels = biannualPoints.filter((p) => p <= lastMonth);
